...
#EN Calibration
fitted.results_model1 <- en.model %>% predict(test.data)

error_model1 <- mean(fitted.results_model1 !=test.data$alc_consumption, na.rm = T)

print(paste('Accuracy [en.model]', 1-error_model1))

testProb <- data.frame(obs = test.data$alc_consumption,
                        pred.logit = error_model1)

calPlotData_model1<- calibration(obs~as.numeric(pred.logit), data = testProb)

xyplot(calPlotData_model1, auto.key = list(columns = 2))
...
#LASSO Calibration
fitted.results_model3 <- lasso_m %>% predict(test.data)

error_model3 <- mean(fitted.results_model3 !=test.data$alc_consumption, na.rm = T)

print(paste('Accuracy [lasso_m]', 1-error_model3))

testProb3 <- data.frame(obs = test.data$alc_consumption,
                        pred.logit3 = error_model3)

calPlotData_model3<- calibration(obs~as.numeric(pred.logit3), data = testProb3)

xyplot(calPlotData_model3, auto.key = list(columns = 2))
```
...
confusionMatrix(en_pred2,as.factor(test.data$alc_consumption))

```
After applying my final model in the test set, the average accuracy was 0.8478 and the sensitivity and specificity was 0.6742 and 1, respectively. The positive predictive value and the negative predictive value were 1 and 0.778, respectively. 
